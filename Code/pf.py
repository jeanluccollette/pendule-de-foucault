...
--------------------
@author Jean-Luc Collette (07/08/2023)
"""
...
def pf_sim(l=30, g=9.81, OT=7.292115e-5, Lat=45,
    ...
    ax.set_xlabel('X (en m) Sud-->Nord'), ax.set_ylabel(
        'Y (en m)  Est-->Ouest'), ax.set_zlabel('Z (en m) Nadir-->Zénith')
    ...
    local_max = np.array([max(theta[n-wq:n+wq+1])
                         for n in range(wq, len(theta)-wq)])
    ...
    plt.figure()
    plt.plot(t_cal, theta, 'r', linewidth=1,
             label='angle de déviation du pendule')
    plt.plot(t_cal[top], theta[top], 'go',
             linewidth=1, label='maxima détectés')
    plt.plot(t_cal[wq:len(theta)-wq], local_max, 'k', linewidth=1,
             label='valeurs maximales dans une fenêtre glissante')
    plt.grid('on')
    ...
    plt.figure()
    plt.plot(t_cal, 180.0*np.arctan2(Y, X)/np.pi, 'b',
             linewidth=1, label='angle du vecteur position')
    plt.plot(temps, angle, 'go', linewidth=1,
             label='valeurs aux maxima de l\'angle de déviation')
    plt.grid('on')
    ...
    plt.plot(t_cal, reg.predict(t_cal.reshape((-1, 1))),
             'b', linewidth=1, label='régression linéaire')
    plt.grid('on')
    ...
    ax.add_patch(Rectangle((0.0, 0.48), 1.0, 0.52,
                 facecolor='blue', alpha=0.5))
    ax.text(0.95, 0.74, 'Paramètres', rotation='vertical',
            fontweight='bold', verticalalignment='center')
    ax.text(0.02, 0.95, f'Longueur du pendule : l={l:.3f} m')
    ax.text(0.02, 0.90, f'Accélération de la pesanteur : g={g:.3f} m/s^2')
    ax.text(
        0.02, 0.85, f'Vitesse de rotation de la Terre : OT={3600*180*OT/np.pi:.3f} °/h')
    ...
    ax.text(
        0.02, 0.65, f'Direction de l\'écart (0° cap nord) : Dir={Dir:.3f} °')
    ax.text(0.02, 0.60, f'Durée de la simulation : Duree={Duree:.3f} s')
    ax.text(0.02, 0.55, f'Pas de temps : Pas={Pas:.3f} s')
    ax.text(
        0.02, 0.50, f'Méthode d\'intégration numérique : methode={methode}')

    ax.add_patch(Rectangle((0.0, 0.03), 1.0, 0.42,
                 facecolor='green', alpha=0.5))
    ax.text(0.95, 0.24, 'Résultats', rotation='vertical',
            fontweight='bold', verticalalignment='center')
    ax.text(
        0.02, 0.40, f'Erreur relative max sur l\'énergie totale : {max(np.absolute((eT-eT[0])/eT[0])):.3e}')
    ax.text(
        0.02, 0.35, f'Vitesse de précession mesurée : {vitesse_p_mes:.3f} °/h')
    ax.text(
        0.02, 0.30, f'Vitesse de précession calculée (Coriolis) : {vitesse_p_cal_coriolis:.3f} °/h')
    ax.text(
        0.02, 0.25, f'Vitesse de précession calculée (Puiseux) : {vitesse_p_cal_puiseux:.3f} °/h')
    ax.text(
        0.02, 0.20, f'Vitesse latérale annulant la précession : {100.0*V_comp:.3f} cm/s')
